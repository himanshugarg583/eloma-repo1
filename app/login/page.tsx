...
export default function LoginPage() {
  ...
  function handleLogin(e: React.FormEvent) {
    ...
    setTimeout(() => {
      setLoading(false)
      alert('Demo: logged in (no auth wired)')
    }, 700)
  }
      ...
      <section className="w-full max-w-md bg-white shadow rounded-lg p-6">
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between gap-4 mt-2">
            ...
            <Link href="/register" className="text-sm text-forest underline">
              Register
            </Link>
          </div>
        </form>

        {/* <p className="mt-4 text-sm text-slate-500">This is a demo login page — no authentication implemented.</p> */}
      </section>
    </main>
  )
}
