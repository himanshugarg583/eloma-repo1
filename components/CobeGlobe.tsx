...
export default function CobeGlobe() {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      globeRef.current = createGlobe(canvas, {
        ...
        onRender(state: Record<string, number>) {
          if (pointerX.current === null) phiRef.current += 0.004;
          state.phi = phiRef.current + drag.current / 200;
        },
    ...
    return () => {
      clearTimeout(tid);
      /* Don't destroy globe here — canvas stays mounted.
         We only destroy on next build() call above. */
    };
  ...
  /* ── Pointer handlers ── */
  const onDown = (x: number) => { pointerX.current = x; };
  const onUp   = ()          => { pointerX.current = null; drag.current = 0; };
  const onMove = (x: number) => {
    if (pointerX.current !== null) drag.current = x - pointerX.current;
  };
    ...
    <div className="flex flex-col items-center w-full select-none">
      ...
      <div className="relative w-full" style={{ aspectRatio: "1/1" }}>
        ...
        <div className="absolute" style={{
          ...
          <canvas
            ref={canvasRef}
            style={{ width: "100%", height: "100%", display: "block", opacity: 0, transition: "opacity 0.6s ease", cursor: "grab" }}
            onPointerDown={(e) => { onDown(e.clientX); (e.target as HTMLElement).style.cursor = "grabbing"; }}
            onPointerUp={(e)   => { onUp();  (e.target as HTMLElement).style.cursor = "grab"; }}
            onPointerOut={(e)  => { onUp();  (e.target as HTMLElement).style.cursor = "grab"; }}
