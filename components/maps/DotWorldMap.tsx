...
export default function DotWorldMap({
    ...
    const ctx = gsap.context(() => {
      ...
      targets.forEach((dest, idx) => {
        ...
        if (pin) {
          tl.to(
            pin,
            { opacity: 3, y: 9, duration: 0.6, ease: "back.out(1.9)" },
            t + 1.6
          );
        }
      });
      ...
      const totalArcsEnd = 0.4 + targets.length * 0.35 + 1.0;
  ...
  const labelOffsets: Partial<
    ...
    >
  > = {
    AU: { x: 16, y: 48, anchor: "start", lineX: 12, lineY: 42 },
    IN: { x: 16, y: 20, anchor: "start", lineX: 12, lineY: 16 },
    US: { x: 16, y: 14, anchor: "start", lineX: 12, lineY: 10 },
    CA: { x: 16, y: -18, anchor: "start", lineX: 12, lineY: -12 },
    UK: { x: 16, y: 14, anchor: "start", lineX: 12, lineY: 10 },
    CN: { x: 16, y: -18, anchor: "start", lineX: 12, lineY: -12 },
    SG: { x: -16, y: 16, anchor: "end", lineX: -12, lineY: 12 },
    AE: { x: -16, y: -18, anchor: "end", lineX: -12, lineY: -12 }
  };
            ...
            <g key={k} data-pin={k} transform={`translate(${p.x} ${p.y})`}>
              ...
              {showLabels ? (
                <line
                  x1={0}
                  y1={0}
                  x2={isOrigin ? 10 : labelOffsets[k]?.lineX ?? 12}
                  y2={isOrigin ? 0 : labelOffsets[k]?.lineY ?? 0}
                  stroke={variant === "ink" ? "rgba(255,255,255,0.55)" : "rgba(12,47,42,0.25)"}
                  strokeWidth={0.8}
                  strokeLinecap="round"
                  opacity={0.9}
                />
              ...
              {showLabels ? (
                <text
                  x={isOrigin ? 14 : labelOffsets[k]?.x ?? 14}
                  y={isOrigin ? 4 : labelOffsets[k]?.y ?? 4}
                  textAnchor={isOrigin ? "start" : labelOffsets[k]?.anchor ?? "start"}
