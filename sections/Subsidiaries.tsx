  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 flex h-screen flex-col justify-center overflow-hidden py-8">
        ...
        <motion.div style={{ x }} className="mt-6 hidden gap-4 px-4 lg:flex">
          {subsidiaries.map((company, idx) => (
            <Link
              key={company.name}
              href="#"
              className="group relative flex w-[min(300px,68vw)] flex-shrink-0 flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card transition-all duration-500 hover:-translate-y-1 hover:border-forest hover:shadow-card-hover"
            >
